...
**
**  Revision 01/03/95 by schaefer@alphanet.ch
**                    Adding heuristic for my needs. Hacking, hacking.
**                    Can be used by defing MIPS_DATE_HACKING.
*/
...
#ifdef MIPS_DATE_HACKING
#   define MAXLINELEN 1000
    { /* Effects with Start != 0 ? */
       char buf1[MAXLINELEN];
       char themonth[MAXLINELEN];
       if (sscanf(p, "%s %s %d %2d:%2d:%2d %d", buf1, themonth, &yyDay, &yyHour,
                                             &yyMinutes, &yySeconds, &yyYear)
           == 7) {
          yyHaveDate = 1;
          yyHaveTime = 1;
       }
       else {
          if (sscanf(p, "%s %d %s %d %2d:%2d %s %s", buf1, &yyDay, themonth,
                                                         &yyYear, &yyHour,
                                                         &yyMinutes,
                                                         buf1, buf1) >= 8) {
             yyHaveDate = 1;
