...
char *s;
{
	...
	if ((buf = malloc(strlen(s) + 1 * sizeof(char))) == NULL) {
		loginf("qp_decode:out of memory:string too long:\"%s\"", s);
		return s;
	}
	for (p = s, q = buf; *p != '\0';) {
		if (*p == '=') {
...
int mode;
{
	...
	if ((buf = malloc(3 * strlen(s) + 1 * sizeof(char))) == NULL) {
		loginf("qp_encode:out of memory:string too long:\"%s\"", s);
		return s;
	}
	for (p = s, q = buf; *p != '\0';) {
		if (*p == '\n')
		...
		else if ((strchr(badchars,*p)) || (*p == '=') ||
			 ((mode==1) && (strchr(badchars2,*p))) )
		{
			if (linelen > 72)
			{
				*q++ = '\n';
...
char *s;
{
	...
	static char buf2[4];
        ...
        if (buf)
                free(buf);
        if ((buf = malloc(3 * strlen(s) + 1 * sizeof(char))) == NULL) {
                loginf("b64_decode:out of memory:string too long:\"%s\"", s);
                return s;
        }
        for (p = s, q = buf; *p != '\0';) {
		for (i=0;i<4;i++) buf2[i]=0x40;	
		for (i=0;((i<4) && (*p != '\0'));) {
			t=(index_hex[(unsigned int)*p]);
			if (*p == '=') buf2[i++]=0x40;
			else if (t != -1) buf2[i++]=(char)t;
			p++;
		}
...
char *s;
{
        ...
        if (buf)
                free(buf);
        if ((buf = malloc(3 * strlen(s) + 1 * sizeof(char))) == NULL) {
                loginf("b64_encode:out of memory:string too long:\"%s\"", s);
                return s;
        }
        for (p = s, q = buf; *p != '\0';) {
		for (i=0;((i<3) && (*p != '\0'));)
		...
		*q++=Base64Code[(((buf2[0] & 0x03) << 4) | ((buf2[1] >> 4) & 0x0f))];
		if (i<2) *q++='=';
 		else *q++=Base64Code[(((buf2[1] & 0x0f) << 2) | ((buf2[2] >> 6) & 0x03))];
		if (i<3) *q++='=';
 		else *q++=Base64Code[(buf2[2] & 0x3f)];
        }
        *q = '\0';
